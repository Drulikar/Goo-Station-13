...
/mob/living/proc/resize(var/new_size, var/animate = TRUE)
	...
	src.add_movespeed_modifier(/datum/movespeed_modifier/resized)
	src.add_or_update_variable_movespeed_modifier(/datum/movespeed_modifier/resized, multiplicative_slowdown = abs(size_multiplier - 1) * 0.8)
